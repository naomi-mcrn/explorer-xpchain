...
function find_richlist(coin, limit, cb) {
  Richlist.findOne({coin: coin}, function(err, richlist) {
    if(richlist) {
      if (limit > 0){
        ...
        var i = 0;
        var max = limit;
        if (richlist.balance.length < limit){
          max = richlist.balance.length;
        }
        for(i=0;i<max;i++){
          rl_bl[i] = richlist.balance[i];
        }
        richlist.balance = rl_bl;
      }
      return cb(richlist);
    } else {
      return cb();
...
function find_richrank(hash, cb) {
  var aggregation = Address.aggregate([
    {"$sort": { "balance" : -1 }},
    {"$group": {"_id": 1,"results": {"$push": {"id": "$_id","a_id": "$a_id","balance": "$balance","received": "$received","sent": "$sent","txs": "$txs" }} }},
  ...
  aggregation.exec(function(err, richlist) {
    if(richlist) {
      return cb(richlist[0]);
    } else {
      return cb();
    }
  });
...
function find_richrank2(hash, cb) {
  find_richlist(settings.coin, -1, function(richlist){
    var i = 0;
    var fi = -1;
    for(i=0;i<richlist.balance.length;i++){
      if (richlist.balance[i].a_id == hash){
        richlist.balance[i].rank = i + 1;
        fi = i;
        break;
      }
    }
    if (fi >= 1){
      return cb(richlist.balance[fi]);
    }else{
      return find_address(hash,cb);//fallback
    }
  });
}
...
module.exports = {
  ...
  get_richlist: function(coin, cb) {
    find_richlist(coin, 2000, function(richlist){
      return cb(richlist);
    });
  },

  get_richrank: function(hash, cb) {
    find_richrank2(hash, function(address){
      return cb(address);
    });
  },
  ...
  update_richlist: function(list, cb){
    if(list == 'received') {
      ...
      Address.find({}).sort({balance: 'desc'}).limit(3000).exec(function(err, addresses){
  ...
  get_distribution: function(richlist, stats, cb){
    var distribution = {
      ...
      t_101_1000: {percent: 0, total: 0},
      t_1001_2000: {percent: 0, total: 0},
      t_2001plus: {percent: 0, total: 0 }
    };
    lib.syncLoop(richlist.balance.length, function (loop) {
      var i = loop.iteration();
      var count = i + 1;
      var percentage = ((richlist.balance[i].balance / 10000) / stats.supply) * 100;
      if (count <= 25 ) {
        distribution.t_1_25.percent = distribution.t_1_25.percent + percentage;
        distribution.t_1_25.total = distribution.t_1_25.total + (richlist.balance[i].balance / 10000);
      }
      if (count <= 50 && count > 25) {
        distribution.t_26_50.percent = distribution.t_26_50.percent + percentage;
        distribution.t_26_50.total = distribution.t_26_50.total + (richlist.balance[i].balance / 10000);
      }
      if (count <= 75 && count > 50) {
        distribution.t_51_75.percent = distribution.t_51_75.percent + percentage;
        distribution.t_51_75.total = distribution.t_51_75.total + (richlist.balance[i].balance / 10000);
      }
      if (count <= 100 && count > 75) {
        distribution.t_76_100.percent = distribution.t_76_100.percent + percentage;
        distribution.t_76_100.total = distribution.t_76_100.total + (richlist.balance[i].balance / 10000);
      }
      if (count <= 1000 && count > 100) {
        distribution.t_101_1000.percent = distribution.t_101_1000.percent + percentage;
        distribution.t_101_1000.total = distribution.t_101_1000.total + (richlist.balance[i].balance / 10000);
      }
      if (count <= 2000 && count > 1000) {
        distribution.t_1001_2000.percent = distribution.t_1001_2000.percent + percentage;
        distribution.t_1001_2000.total = distribution.t_1001_2000.total + (richlist.balance[i].balance / 10000);
      }
      loop.next();
    }, function(){
      distribution.t_2001plus.percent = parseFloat(100 - distribution.t_1001_2000.percent - distribution.t_101_1000.percent - distribution.t_76_100.percent - distribution.t_51_75.percent - distribution.t_26_50.percent - distribution.t_1_25.percent).toFixed(2);
      distribution.t_2001plus.total = parseFloat(distribution.supply - distribution.t_1001_2000.total - distribution.t_101_1000.total - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total).toFixed(4);

      distribution.t_1_25.percent = parseFloat(distribution.t_1_25.percent).toFixed(2);
      distribution.t_1_25.total = parseFloat(distribution.t_1_25.total).toFixed(4);
      distribution.t_26_50.percent = parseFloat(distribution.t_26_50.percent).toFixed(2);
      distribution.t_26_50.total = parseFloat(distribution.t_26_50.total).toFixed(4);
      distribution.t_51_75.percent = parseFloat(distribution.t_51_75.percent).toFixed(2);
      distribution.t_51_75.total = parseFloat(distribution.t_51_75.total).toFixed(4);
      distribution.t_76_100.percent = parseFloat(distribution.t_76_100.percent).toFixed(2);
      distribution.t_76_100.total = parseFloat(distribution.t_76_100.total).toFixed(4);
      distribution.t_101_1000.percent = parseFloat(distribution.t_101_1000.percent).toFixed(2);
      distribution.t_101_1000.total = parseFloat(distribution.t_101_1000.total).toFixed(4);
      distribution.t_1001_2000.percent = parseFloat(distribution.t_1001_2000.percent).toFixed(2);
      distribution.t_1001_2000.total = parseFloat(distribution.t_1001_2000.total).toFixed(4);
